...
class MRNATOK:
    ...
    def clean_str(self, str_in):
        ...
        output_string = str_in.upper()
        output_string = re.sub("T", 'U', output_string)
    ...
    def get_ids_given_three_seg(self, five, three, codons):
        five = self.clean_str(five)
        three = self.clean_str(three)
        codons = self.clean_str(codons)
        L_codon = len(codons)
        ret_first = [self.map_nuc[xi]+self.offset_first for xi in five]
        ret_last = [self.map_nuc[xi]+self.offset_last for xi in three]
        ret_codon = [self.triple2id_py(codons[ti:(ti+3)], self.map_codon_id) + self.offset_codon for ti in range(0, L_codon, 3)] # TODO
...
class DebugTensorDataset(LocalDataset):
    ...
    def __getitem__(self, index: int):
        ...
        L_5 = ( (dat>=4) * (dat<=8)).sum()
        L_3 = ( (dat>=134) * (dat<=138)).sum()
        L_codon = ( (dat>=9) * (dat<=133)).sum()
        return len(dat), L_5, L_3, L_codon